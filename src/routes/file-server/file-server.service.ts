...
class FileServerService {
    private validateFilePath(folderName: string, fileName: string): string {
      ...
      // path to file
      let file = path.join(dir, folder, fileName)
    ...
    public fileServer (folder: string, fileName: string, req: Request, res: Response, next: NextFunction) {
      const file: string = this.validateFilePath(folder, fileName);

      // Get mime type.
      let type: string = mime[path.extname(file).slice(1)] || 'text/plain';
